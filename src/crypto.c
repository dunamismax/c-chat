...
cchat_error_t encrypt_message(const char* message, const unsigned char* recipient_public_key, 
    ...
    if (crypto_box_seal(encrypted, (const unsigned char*)message, message_len, recipient_public_key) != 0) {
...
cchat_error_t decrypt_message(const unsigned char* encrypted, size_t encrypted_len,
    ...
    if (!encrypted || !private_key || !message || !message_len || encrypted_len == 0) {
        return CCHAT_ERROR_INVALID_ARGS;
    }
    ...
    if (crypto_box_seal_open(decrypted_buffer, encrypted, encrypted_len, public_key, private_key) != 0) {
...
cchat_error_t save_keys_to_file(const char* username, const unsigned char* public_key, 
    ...
    char keys_path[512];
    snprintf(keys_path, sizeof(keys_path), "%s/%s/%s.keys", home_dir, KEYS_DIR, username);
    ...
    if (crypto_secretbox_easy(encrypted_private, private_key, PRIVATE_KEY_SIZE, nonce, derived_key) != 0) {
        free(home_dir);
        secure_zero_memory(derived_key, sizeof(derived_key));
        return CCHAT_ERROR_ENCRYPTION;
    }
    ...
    if (fwrite(salt, 1, sizeof(salt), file) != sizeof(salt) ||
        ...
        fwrite(encrypted_private, 1, sizeof(encrypted_private), file) != sizeof(encrypted_private)) {
        fclose(file);
        unlink(keys_path);
        free(home_dir);
        secure_zero_memory(derived_key, sizeof(derived_key));
...
cchat_error_t load_keys_from_file(const char* username, unsigned char* public_key,
    ...
    if (fread(salt, 1, sizeof(salt), file) != sizeof(salt) ||
        ...
        fread(encrypted_private, 1, sizeof(encrypted_private), file) != sizeof(encrypted_private)) {
        fclose(file);
        free(home_dir);
        return CCHAT_ERROR_FILE_IO;
    }
    ...
    // Decrypt private key
    if (crypto_secretbox_open_easy(private_key, encrypted_private, sizeof(encrypted_private), nonce, derived_key) != 0) {
        secure_zero_memory(derived_key, sizeof(derived_key));
        return CCHAT_ERROR_DECRYPTION;
    }
...
cchat_error_t derive_key_from_password(const char* password, const unsigned char* salt,
    ...
    if (crypto_pwhash(derived_key, DERIVED_KEY_SIZE,
