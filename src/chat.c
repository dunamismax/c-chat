...
cchat_error_t start_chat(const char* username) {
    ...
    if (!session_keys_loaded) {
        char password[256];
        get_password_input("Enter your password to start secure chat: ", password, sizeof(password));
        
        unsigned char public_key_temp[PUBLIC_KEY_SIZE];
        cchat_error_t result = load_keys_from_file(current_session.current_user.username, 
                                                  public_key_temp, current_user_private_key, password);
    ...
    if (generate_keypair(chat_partner_public_key, temp_private) != CCHAT_SUCCESS) {
    ...
    memcpy(current_session.chat_partner.public_key, chat_partner_public_key, PUBLIC_KEY_SIZE);
    ...
    while (current_session.is_in_chat) {
        ...
        if (strcmp(message, "/exit") == 0) {
            ...
            // Clear session keys
            secure_zero_memory(current_user_private_key, sizeof(current_user_private_key));
            secure_zero_memory(chat_partner_public_key, sizeof(chat_partner_public_key));
...
cchat_error_t send_message(const char* message) {
    ...
    cchat_error_t result = encrypt_message(message, chat_partner_public_key, encrypted, &encrypted_len);
    if (result != CCHAT_SUCCESS) {
...
cchat_error_t receive_messages(void) {
    ...
    // Encrypt with our own public key for demo (in reality, this would come from server)
    unsigned char our_public[PUBLIC_KEY_SIZE];
    crypto_scalarmult_base(our_public, current_user_private_key);
    
    if (encrypt_message(demo_message, our_public, encrypted, &encrypted_len) != CCHAT_SUCCESS) {
    ...
    cchat_error_t result = decrypt_message(encrypted, encrypted_len, current_user_private_key, decrypted, &decrypted_len);
    if (result != CCHAT_SUCCESS) {
